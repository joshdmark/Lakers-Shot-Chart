...
##### LAKERS ROSTER
lakers_roster <- readxl::read_xlsx("Documents/Sports/Lakers/lakers_roster_ids.xlsx", 
                                   sheet = 'LAL Roster') %>% 
  data.frame() #%>% 
  # filter(player != 'Devontae Cacok')

# lakers_roster %>% head()
lakers_player_ids <- lakers_roster$person_id
team_shot_data <- data.frame()
for(id in lakers_player_ids){
  print(id)
  tmp_shot_data <- fetch_shots_by_player_id_and_season(player_id = id,
                                             season = '2020-21',
                                             # season_type = 'Regular Season')[1] %>%
                                             season_type = 'Playoffs')[1] %>%
    data.frame() %>%
    mutate(person_id_shot_chart = id) %>% 
    filter(player.team_name == 'Los Angeles Lakers') ## new 
  names(tmp_shot_data) <- str_replace_all(names(tmp_shot_data), 'player.', '')
  team_shot_data <- rbind(team_shot_data, tmp_shot_data)
  Sys.sleep(5)
}
team_shot_data <- team_shot_data %>% 
  mutate(shot_id = 1:n(), 
         shot_id = paste('s', shot_id, sep = ''), 
         loc_x2 = loc_x * 2)

## clean data for Tableau
team_shot_data <- team_shot_data %>% 
  mutate(opponent = ifelse(htm == 'LAL', vtm, htm), 
         fga = 1, 
         fgm = ifelse(shot_made_flag == 'made', 1, 0), 
         x3fga = as.numeric(shot_type == '3PT Field Goal'),
         x3fgm = as.numeric(shot_type == '3PT Field Goal' & 
                              shot_made_flag == 'made'), 
         x2fga = as.numeric(shot_type == '2PT Field Goal'),
         x2fgm = as.numeric(shot_type == '2PT Field Goal' & 
                              shot_made_flag == 'made'))
## add uniforms
lakers_uniforms <- readxl::read_xlsx("Documents/Sports/Lakers/lakers_uniforms.xlsx", 
                                   sheet = 1) %>% 
  data.frame() %>% 
  mutate(game_dt = lubridate::ymd(game_dt))

## add uniform to team_shot_data
team_shot_data <- sqldf('select t.*, l.laker_uniform
                    from team_shot_data t 
                    join lakers_uniforms l 
                    on t.game_date = l.game_dt')

## output regular season data as RDS file type
# write_rds(team_shot_data, 'Documents/Sports/Lakers/team_shot_data_regular_season.RDS')

## add back in regular season data
# reg_season_data <- data.table::fread('Documents/Sports/Lakers/team_shot_data_regular_season.csv') %>%
reg_season_data <- read_rds('Documents/Sports/Lakers/team_shot_data_regular_season.RDS') %>%
  data.frame() %>%
  mutate(season_type = 'Regular Season')

## playoff data
playoff_data <- team_shot_data %>%
  mutate(season_type = 'Playoffs')

## combine playoffs & regular season
team_shot_data <- bind_rows(reg_season_data, playoff_data)

## write output csv 
data.table::fwrite(team_shot_data, 'Documents/Sports/Lakers/team_shot_data.csv')



#### 3PT Contest shooters
# contestants <- c('Devin Booker', 'Jaylen Brown', 'Stephen Curry',
#                  'Zach Lavine', 'Donovan Mitchell', 'Jayson Tatum', 
#                  'Mike Conley')
# x3pt_df <- data.frame()
# for (c in contestants){
#   print(c)
#   tmp_df <- find_player_by_name(c) %>% data.frame() %>%
#     select(display_first_last, person_id)
#   x3pt_df <- bind_rows(x3pt_df, tmp_df)
# }
# 
# shot_data <- data.frame()
# for (p in 1:nrow(x3pt_df)){
#   id <- x3pt_df$person_id[p]
#   tmp_shot_data <- fetch_shots_by_player_id_and_season(player_id = id,
#                                                        season = '2020-21',
#                                                        season_type = 'Regular Season')[1] %>%
#     data.frame() %>%
...
#          shot_id = paste('s', shot_id, sep = ''),
#          loc_x2 = loc_x * 2)
# data.table::fwrite(shot_data, 'Documents/Sports/NBA 3PT Contest/shot_data.csv')
